                            ...
                            <div class="card-body" style="padding: 0.5rem;">
                                <div class="row" style="margin: 0;">
                                    <div class="col-lg-2 col-md-2 col-sm-2 mobile-width border-right-blue-grey border-right-lighten-5">
                                        <div class="btn-group">
                                            <button class="btn btn-secondary btn-lg dropdown-toggle pl-0" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="border:none;background-color: white;color: black;font-weight: bold;">
                                                All Delete Retail Pending Quantity
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-1 col-md-1 col-sm-1 mobile-width">                                 
                                            <a href="add_del_retail_pending_qua.php" style="float: left;"><button type="button" class="btn btn-success " ><i class="fa fa-plus"></i> NEW</button></a>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered responsive" id="tbl">
                                ...
                                <tbody>
                                    <?php 
                                        ...
                                        // if($flag==0)
                                        // {
                                            $sql = "SELECT * FROM retail_delete_pending_qty WHERE flag='$flag'";
                                    ...
                                    $sql = "SELECT * FROM retail_proforma WHERE id_pk='".$j['po_no']."'";
                                    $lk = mysqli_query($db,$sql);
                                    while($m = mysqli_fetch_array($lk))
                                    {
                                        $order_no = $m['order_no'];
                                    }
                                    $sql = "SELECT * FROM mstr_retail_customer WHERE retail_cust_idpk='".$j['customer_id']."'";
                                    $lk = mysqli_query($db,$sql);
                                    while($m = mysqli_fetch_array($lk))
                                    {
                                        $retail_cust_name = $m['retail_cust_name'];
                                    }
                            ?> 
                            <tr style="cursor: pointer"> 
                                ...
                                <td style="display: flex;">
                                    <!-- <a href="edit_del_retail_pending_qua.php?bfiirbfr=dnfvibi&njsbuisv=sdmdfuiefb&id=<?php echo $j['d_id_pk'];?>&dhjsh=efnb&bbdhjrb=esfbehvf&ejgfebf=sdfbuyeg" class="btn btn-success btn-sm">
                                        <i class="fa fa-pencil" aria-hidden="true"></i>
                                          
                                    </a> -->
                                    
                                    <a onclick="return confirm('Are you sure you want to delete?')" href="../../api/retail/delete_del_retail_pending_qua.php?bfiirbfr=dnfvibi&njsbuisv=sdmdfuiefb&id=<?php echo $j['d_id_pk'];?>&dhjsh=efnb&bbdhjrb=esfbehvf&ejgfebf=sdfbuyeg" class="btn btn-danger btn-sm">
                                        <i class="fa fa-trash" aria-hidden="true"></i>
                                          
                                    </a>
                                </td>
                                <td><?php echo $order_no; ?></td>
                                <td><?php echo $j['add_date']; ?></td>
                                <td><?php echo $retail_cust_name; ?></td>
	...
	$(document).ready(function()
  {
    $('#tbl').DataTable( {
        ...
        "createdRow": function(row, data, dataIndex){
           // alert(data[4])
            if(data[5]==`<span class="text-danger">InActive</span>`)
