<?php 
    ...
    $newRawItemArray1 = json_decode($dc_id_array, true);
    $cgst_amt = 0;
    $igst_amt_final = 0;
    ...
    for($j = 0;$j<$length1;$j++)
    {   
        $dcid = $newRawItemArray1[$j];

        $sql = "SELECT * FROM wholsale_delivery_challan_items as woi,mstr_item as i 
        WHERE woi.item_id_fk= i.item_id_pk AND woi.dc_id_fk='$dcid'
        ";
        ...
        while ($row = mysqli_fetch_array($result))
        {
            ...
            //qty
            $obj['4'] = '<input type="text" id="quantity-'.$i.'" class="form-control" value="'.$row['deliverd_qty'].'" onkeyup="get_quantity_value(this.id);" >';//$row['3'];//qty
            //sqft
            ...
            //amt
            $obj['9'] = '<input type="text" id="amount-'.$i.'" class="form-control" value="'.$row['amount'].'" >';//$row['8'];//dis_per
            //remark
            $obj['10'] = '<input type="text" id="remark-'.$i.'" class="form-control" value="'.$row['remark'].'" >';//$row['9'];//dic_rs
            //hidden item id
            $obj['11'] = '<input type="text" id="hidden_id-'.$i.'" class="form-control d-none" value="'.$row['item_id_fk'].'" >';//$row['9'];//dic_rs
            //GST %
            $obj['12'] = '<input type="text" id="gst_per-'.$i.'" class="form-control" value="'.$gst_group1.'" >';//$row['9'];//dic_rs
            //cGST amt
            $obj['13'] = '<input type="text" id="cgst_amt-'.$i.'" class="form-control" value="'.$cgst_amt.'" >';//$row['9'];//dic_rs
            //sGST %
            $obj['14'] = '<input type="text" id="agst_amt-'.$i.'" class="form-control" value="'.$cgst_amt.'" >';//$row['9'];//dic_rs
            //igst amt
            $obj['15'] = '<input type="text" id="igst_amt-'.$i.'" class="form-control" value="'.$igst_amt_final.'" >';//$row['9'];//dic_rs
            //dc id pk
            $obj['16'] = '<input type="text" id="dc_id_hidden-'.$i.'" class="form-control d-none" value="'.$dcid.'" >';//$row['9'];//dic_rs
            $i++;
            array_push($data, $obj);
