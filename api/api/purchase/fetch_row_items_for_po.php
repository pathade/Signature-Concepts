<?php 
    ...
    for($if=0;$if<$count;$if++)
    {
        ...
        while($row = mysqli_fetch_array($g))
        {
            ...
            //Qty
            $obj['3'] = '<input type="text" id="quantity'.$i.'" class="form-control" value="0" onkeyup="get_quantity_value(this.id)">';//account
            //Sqft
            ...
            //Discount Rs.
            $obj['7'] = '<input type="text" id="table_discount_rs'.$i.'" class="form-control" value="0" onkeyup="get_row_discount_value(this.id);">';//$row['4'];//length
            //Amount
            $obj['8'] = '<input type="text" id="amount'.$i.'" class="form-control" value="0" style="border: white;background-color: white;">';//$row['5'];//breadth
            //Remark
            $obj['9'] = '<input type="text" onkeyup="" id="remark'.$i.'" class="form-control" value="" >';//$row['6'];//sqft
            //Product Id
            $obj['10'] = '<input type="text" id="product_id-'.$i.'" class="form-control" value="'.$row['item_id_pk'].'" style="display: none" />';//$row['3'];//qty
            //PO checkbox
