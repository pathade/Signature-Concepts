<?php
    ...
    $n = "SELECT * FROM wholesale_delivery_challan WHERE  wd_ch_id_pk='".$_GET['id']."'";
    $m = mysqli_query($db,$n);
        ...
        while($b = mysqli_fetch_array($m))
        {
            ...
            while($nr = mysqli_fetch_array($nres))
            {
                ...
                if($b == 1)
                {
                    ...
                    $dsql = "UPDATE wholesale_delivery_challan SET delete_status='1' where  wd_ch_id_pk='".$_GET['id']."'";
                    $k = mysqli_query($db,$dsql);
