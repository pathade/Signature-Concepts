...
// $id = $_GET['this_id'];
  ...
  function Header()
 {
    ...
    $edit_id =1;    /// $_GET['id'];
    $sql_charges = "SELECT *,wo.pono, wo.remark
                    FROM wholesale_tax_invoice AS wi, wholesale_work_order AS wo
    ...
    $row = mysqli_fetch_row($result_charges);
     ...
     $this->Cell(191,29,'   ',1,1,'L',0,false);
     ...
     $this->SetFont('Arial','B',9);
     $this->Cell(25,8,$row['50'],0,1,'L',0,false);
     ...
     $sql_charges1 ="SELECT twc.wc_id_pk, twc.cust_name, twc.office_address,
                     twc.mob_number,twc.gst_no,wti.tax_id_pk, twcs.wc_id_fk, twcs.site_address, wti.remark
