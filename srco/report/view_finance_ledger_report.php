                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group row" style="margin-left: 0px;">
                                                ...
                                                <div class="col-md-5">
                                                    <div class="form-group row">
                                                        <label class="col-md-3 label-control" for="userinput1">Category</label>
                                                        <div class="col-md-9">
                                                            <select class="select2 form-control block" id="category" class="select2" data-toggle="select2" name="category" onchange="getName(this.value)">
                                                                <option value="" disabled selected>Select </option>
                                                                <option value="wholesale">Wholesale Ledger</option>
                                                                <option value="retail">Retail Ledger</option>
                                                ...
                                                <div class="col-md-5">
                                                    <div class="form-group row">
                                                        <label class="col-md-3 label-control" for="userinput1">Select Name</label>
                                                        <div class="col-md-9">
                                                            <select class="select2 form-control block" id="names" class="select2" data-toggle="select2" name="names">
                                                                <option value="" disabled selected>Select </option>
                                                ...
                                                <div class="col-md-1 mt-1">
                                                    <div class="form-group row">
                                                        <input class="checkbox" type="checkbox" id="all_names" name="all_names" />&nbsp;&nbsp;All
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
function getName(val)
{
    // alert(val);
    $.ajax({
        url: '../../api/global/get_name_for_ledger.php',
        type: 'POST',
        data: { 'val': val },
...
<script>
    function show_data()
    {
        ...
        var all_names = document.getElementById('all_names').checked;
        ...
        if(all_names == false && names == '')
        {
            alert('please select name OR check all button');
            return
        }
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.each(data, function(index) 
                {
                    ...
                    var against=`<p>${data[index].against}</p>`;
                    var o_bal=`<p>${data[index].o_bal}</p>`;
                    var debit=`<p class="text-danger">${data[index].debit}</p>`;
                    var credit= `<p class="text-success">${data[index].credit}</p>`;
                    var closing_detail= `<p>${data[index].closing_detail}</p>`;
