	                    	...
	                    	<div class="form-body">
                                <div class="row" style="visibility: hidden;">
                                    <div class="col-md-6">
                                        <?php
                                            if ($flag == 0) {
                                                $sql = "SELECT * FROM mstr_data_series where name='exp_payment_advice' and flag= '0'";
                                            }
                                            else {
                                                $sql = "SELECT * FROM mstr_data_series where name='exp_payment_advice' and flag= '1'";
                                            }
...
{   
   ...
   table.clear().draw();
    ...
    $.ajax({
        ...
        {
             var select=`<input type="checkbox" checked value="${data[index].start}" onchange="tax_invoice_click(this.id)">`;  
  ...
  function save_data()
{
    ...
    var padv_no = document.getElementById('padv_no').value; 
    ...
    && authorised_by!="" && pay_date!="" )
    {
        $.ajax(
        {
            ...
            url: "../../api/expense/save_exp_payment_advice.php",
            type: 'POST',
            data : $('#payment_advice_form').serialize()+ "&newRawItemArray="+ newRawItemArray+ "&vendor_id_fk=" + vendor_id_fk+
            "&type_vari=" + type_vari+ "&authorised_by=" + authorised_by+ "&padv_no=" + padv_no,
            dataType:'text',  
            success: function(data)
            { 
