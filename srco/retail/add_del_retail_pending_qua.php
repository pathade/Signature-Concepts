		...
		<div class="col-md-12">
	        <div class="card">
				<div class="card-header">
	                <h4 class="card-title" id="horz-layout-colored-controls" style="font-size:40px;">Add Delete Retail Pending Quantity</h4>
                    <hr>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
			                        	<div class="form-group row ">
                                            ...
                                            <!-- <input type="radio" name="pay_type" id="credit_note" style="height: 14px;width: 14px;" /> &nbsp;Credit Note -->
                                        </div>											
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group row">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
										<div class="form-group row">
				                        	...
				                        	<div class="col-md-9 ">
                                                <input type="text" id="cheque_no" class="form-control " placeholder="0" name="cheque_no" value="" >
                                            </div>
                                        </div>
                                    </div>
		                        </div>
       ...
       // alert("po_no:"+po_no);
        ...
        $.ajax({
            ...
            $.each(data, function(index)  
            {
                ...
                var uom = `<select id="uom-${i}" class="form-control"><option>PCS</option><option>BOX</option><option>SQFT</option><option>BAGS</option></select>`;
...
$(document).ready(function () {
    $('#add_purchase_order').on('click', function () {
        ...
        $("#tbl input[type=checkbox]:checked").each(function () {
            ...
            uom = row.cells[4].childNodes[0].value;
            ...
            rate = row.cells[8].childNodes[0].value;
            final_net_amt = row.cells[9].childNodes[0].value;
            reduced_amt = row.cells[10].childNodes[0].value;
            item_id_fk = row.cells[11].childNodes[0].value;
        ...
        if($("input:radio[name='pay_type']").is(":checked") && customer!="" && po_no!="" && deleted_amt!="") 
        {
            ...
            if(payval == "cheque_back")
            {
                if(bank !="" && account_no!="" && cheque_no!="")
                {
                    $.ajax(
                    {
                        url: "../../api/retail/add_delete_pending_qty.php",
                        type: 'POST',
                        data : $('#form1').serialize() + "&newRawItemArray=" + newRawItemArray + '&account_no=' +account_no +'&bank='+bank,
                        dataType:'text',  
                        success: function(data)  
                        { 
                            //alert("data:"+data);
