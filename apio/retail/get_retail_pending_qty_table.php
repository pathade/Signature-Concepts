...
if($_SERVER['REQUEST_METHOD']=='POST')
{
  ...
  $sql = "SELECT * FROM `retail_pending_qty` as rp ,mstr_item as i 
  where rp.item_id_fk = i.item_id_pk AND rproforma_id_fk='$po_no'";
  ...
  while($row=mysqli_fetch_assoc($result))
  {
    //echo "1111111111111111".$item123 = $row['item_id_fk'];
     ...
     $d = "SELECT * FROM retail_proforma WHERE id_pk='$po_no'";
      ...
      $obj['start'] = $rproforma_item_id_fk;
      $obj['pono'] = $order_no;
      $obj['item_name'] = $res['0'];
      $obj['size'] = $row['size'];
      $obj['oqty'] = $row['order_qty'];
      $obj['pqty'] = $row['remain_qty'];
      $obj['dqty'] = 0;
      $obj['rate'] = $rate;
      $obj['final_amt'] = $net_amt;
      $obj['reduced_amt'] = 0;
