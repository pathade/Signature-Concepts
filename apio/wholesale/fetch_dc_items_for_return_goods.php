<?php 
    ...
    while ($row = mysqli_fetch_array($result))
    {
        ...
        //sqft
        $obj['7'] = '<input type="text" id="sqrft-'.$i.'" class="form-control" value="'.$row['sqrfit'].'" style="border: white;background-color: white;">';//$row['4'];//length
        //rate
        $obj['8'] = '<input type="text" id="rate-'.$i.'" class="form-control" value="'.$row['rate'].'" style="border: white;background-color: white;">';//$row['5'];//breadth
        //discount
        $obj['9'] = '<input type="text" onkeyup="get_row_discount_value1(this.id)" id="table_discount_per-'.$i.'" class="form-control" value="'.$row['disc_per'].'" style="border: white;background-color: white;">';//$row['6'];//sqft
        //disc rs
        $obj['10'] = '<input type="text" id="table_discount_rs-'.$i.'" onkeyup="get_row_discount_value(this.id)" class="form-control" value="'.$row['disc_rs'].'" style="border: white;background-color: white;" readonly>';//$row['7'];//rate
        //amt
        $obj['11'] = '<input type="text" id="amount-'.$i.'" readonly="" class="form-control" value="'.$row['amount'].'" >';//$row['8'];//dis_per
        //return amount
        $obj['12'] = '<input type="text" id="return_amt-'.$i.'" class="form-control" value="0" onkeyup="get_quantity_value(this.id);" >';//$row['3'];//qty
        //remark
        $obj['13'] = '<input type="text" id="remark-'.$i.'" readonly = "" class="form-control" value="'.$row['remark'].'" >';//$row['9'];//dic_rs
        //hidden item id
        $obj['14'] = '<input type="text" id="hidden_id-'.$i.'" class="form-control" value="'.$row['item_id_fk'].'" >';//$row['9'];//dic_rs
        //GST %
