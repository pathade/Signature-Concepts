<?php 
    ...
    SET `Tax_inv_no`='$tax_inv_no',`cust_id_fk`='$customer',
    `work_id_fk`='$work_no_select_hidden',`bill_date`='$bill_date1',
    `remark`='$remark',
    `dc_no_list`='0',`discount_amt`='$final_disc_amt',`other_disc_amt`='$other_disc_amt',
    `transaction_amt`='$final_trans_amt',`load_unload`='$load_unload',`other`='$other',
    `gross_amt`='$gross_amt',`tax_amt`='$tax_amt',`net_amt`='$net_amt',
    `billing_name`='$billing_name',
    `financial_year`='$fin_yr' WHERE `tax_id_pk`='$edit'";
    ...
    if($res == 1)
    {
        $dsql = "DELETE FROM wholesale_tax_invoice_items WHERE tax_id_fk='$edit'";
        ...
        {   
            ...
            VALUES ('$edit','$product_category','$item_id_fk',
            ...
            $check_item = "SELECT * FROM wholesale_stock_table WHERE item_id_fk='$item_id_fk'";
            $res_item = mysqli_query($db, $check_item) or die(mysqli_error($db));
            if(mysqli_num_rows($res_item) < 1)
            {
             $sql1 = "INSERT INTO wholesale_stock_table(item_id_fk, total_qty) VALUES('$item_id_fk', '$quantity')";
             $res = mysqli_query($db, $sql1) or die(mysqli_error($db));
            }
            else
            {
             $sql1 = "UPDATE wholesale_stock_table SET total_qty='$quantity' WHERE item_id_fk='$item_id_fk'";
             $res = mysqli_query($db, $sql1) or die(mysqli_error($db));
            }
            ...
            if($res1 == 1)
            {
                ...
                // $update_no1 = $first."".$second."/".$update_no;
                ...
                // $update_no = $sr_no + 1;
                // $data_series_update_sql = "UPDATE mstr_data_series SET sr_no='$update_no1' WHERE name='wholesale_tax_invoice'";
